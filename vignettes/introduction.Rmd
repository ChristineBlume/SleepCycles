---
title: "Introduction to the SleepCycles"
...
## Worked example
First, we install (not shown here) and load the package if we haven't done so.
```{r setup}
library(SleepCycles)
```
...
## create a new directory in the temporary directory (don't worry, it will automatically be deleted when you restart your computer)
...
## we set the directory back to the one we were using before as we were just working in the temp directory.
#setwd(olddir)
```